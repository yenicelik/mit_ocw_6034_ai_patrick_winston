...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Product) and isinstance(expr2, Product):
        # Should just append a list and output that list
        out = two_products(expr1, expr2)

    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        # Should Multiply Both Terms with the product
        out = one_sum_one_product(expr1, expr2)
        ...
        # Should Multiply Both Terms with the product
       out = two_sums(expr1, expr2)

    return out

def two_products(expr1, expr2):
    output = Product(expr1 + expr2)

    return output

def one_sum_one_product(p_sum, p_product):
    new_terms = []
    for term in p_sum:

...
#
# rule1 = IF( AND( '(?x) is a Norwegian Blue parrot',             // Polly is not dead
#                  '(?x) is motionless' ),                        //
#             THEN( '(?x) is not dead' ) )                        //  
#
...
forward_chain([transitive_rule], poker_data)
...
family_rules = [
                ...
                IF( AND('parent (?x) (?y)', 'parent (?z) (?w)', OR('brother (?x) (?z)', 'sister (?x) (?z)'), NOT('same-identity (?y) (?w)')), THEN('cousin (?y) (?w)', 'cousin (?w) (?y)' )),

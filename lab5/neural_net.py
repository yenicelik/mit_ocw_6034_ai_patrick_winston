...
def make_neural_net_challenging():
    ...
    wD = Weight('wD', random_weight() ) #Two additinal elements between the current two layers and output layer
    wE = Weight('wE', random_weight() ) #Two additinal elements between the current two layers and output layer
    ...
    wDC = Weight('wDC', random_weight() ) 
    wEC = Weight('wEC', random_weight() )
    ...
    E = Neuron('E', [i0, A, B], [wE, wAE, wBE])

    C = Neuron('C', [i0, D, E], [wC, wDC, wEC])

    P = PerformanceElem(C, 0.0)

    net = Network(P, [A, B, D, E, C])
    return net
